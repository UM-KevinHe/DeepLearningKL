...
class NewlyDefinedLoss(nn.Module):
  ...
  def newly_defined_loss(self, phi, x_train, idx_durations, events):
    ...
    zeros_train = np.zeros((time.shape[0], 21))  # 21 - 1 = 20
...
def cross_validation_eta(df_local, eta_list, model_prior,
  ...
  for eta in eta_list:
    ...
    for train_index, test_index in kf.split(df_train):
      ...
      concordance_td, integrated_brier_score, integrated_nbll = evaluation_metrics(x_test, durations_test, events_test,
                                                                                   model)
